...
#define RAM_SIZE	(1 << ADDRESS_SIZE) //////// Copy from mem.h
...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	 ...
	 * */
	int total_pages = num_pages;
	int avail_page[total_pages];
	int i = 0;
	while(i<=NUM_PAGES && num_pages > 0){
		if( ! _mem_stat[i].proc){
			avail_page[total_pages-num_pages] = i;
			num_pages--;
		}
		if(num_pages == 0){
			mem_avail = 1;
	...
	// Check if virtual memory space is enough
	if (proc->bp + total_pages* PAGE_SIZE > (1<<ADDRESS_SIZE)){
		mem_avail = 0;
	}
	...
	if (mem_avail) {
		...
		for( int i = 0; i <total_pages; i++){
			// update proc -> size
			int segIndex = get_first_lv(addr_vir_mem);
			...
			// update v_index and p_index of page_table_t
			int pageSize = proc->seg_table->table[segIndex].pages->size - 1;
			...
			proc->seg_table->table[segIndex].pages->table[pageSize].p_index = avail_page[i];

			// update virtual address for the next use
			addr_vir_mem += PAGE_SIZE;
			proc->seg_table->size++;

			// update _mem_stat.next
			if (i == total_pages -1){
